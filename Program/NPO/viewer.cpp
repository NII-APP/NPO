...
void Viewer::addModel() {
    ...
    if (QFile::exists(mode)) {
        MeshForm* enother = new MeshForm(*forAdd);
...
void Viewer::setMesh(const FEM *g) {
    ...
    try {
        const MeshForm& full(dynamic_cast<const MeshForm&>(*g));
        setMode(1);
        if (formSelector) {
            formSelector->show();
...
void Viewer::setMode(int m) {
    ...
    if (formSubLabel) {
        if (g.size() == 1 && dynamic_cast<const MeshForm*>(g.first())) {
            formSubLabel->setText(QString::number(dynamic_cast<const MeshForm*>(g.first())->frequency(m))
    ...
    if (colorizeBundle) {
        ...
        MeshForm* model(dynamic_cast<MeshForm*>(Application::project()->modelsList()[i]));
        if (model) {
            model->colorize(m);
        }
    }
